...
export async function runAccessibilityScan({ startUrl, maxDepth = 3, maxScans = 50, concurrency = 5 }) {
    const axeSource = await readFile(path.join(__dirname, 'node_modules', 'axe-core', 'axe.min.js'), 'utf8');
        ...
        try {
            ...
            const links = await page.evaluate((originStr) => {
                return Array.from(document.querySelectorAll('a'))
                    .map(el => el.href.trim())
                    .filter(href => href.startsWith(originStr) &&
                    !href.includes('#') &&
                    !href.includes('?') &&
                    !href.endsWith('.pdf') &&
                    !href.endsWith('.jpg') &&
    ...
    while (currentLevel.length > 0 && scanCount < maxScans) {
        ...
        for (const { url, depth } of currentLevel) {
            if (depth >= maxDepth || scanCount >= maxScans)
                continue;
